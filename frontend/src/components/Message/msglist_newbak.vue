<template>
    <div class="wxchat-container" :style="{backgroundColor: wrapBg}">
        <div class="window" id="window-view-container" :style="{maxHeight: maxHeight + 'px', width: width +'px'}">
            <!-- data is empty -->
            <div class="loading" v-if="getNewMsg.length==0">
            ...
            <!-- main -->
            <ScrollLoader :minHeight="minHeight" @scroll-to-top="refresh" @scroll-to-botton="infinite" class="container-main" v-if="getNewMsg.length>0" :style="{maxHeight: maxHeight-50 + 'px'}">
                <div class="message">
                    {{msg}}
                    <ul>
                        <li v-for="(message, index) in getNewMsg" :key="message.UserId" :class="message.direction=='send'?'an-move-right':'an-move-left'">
                            ...
                            <div :class="'main' + (message.direction=='send'?' self':'')" v-else>
                                <img class="avatar" width="45" height="45" :src="message.direction=='send'? ownerAvatarUrl: contactAvatarUrl">
                                <!-- 文本 -->
                                <div class="text" v-emotion="message.cont" v-if="message.type==1"></div>

                                <!-- 图片 -->
                                <div class="text" v-else-if="message.type==2">
                                    <img :src="message.cont" class="image" alt="聊天图片">
                                </div>

                                <!-- 其他 -->
                                <div class="text" v-else-if="message.type!=10000" v-text="'[暂未支持的消息类型:'+message.direction+ message.type +']\n\r' + message.cont">
    ...
    export default {
        ...
        data() {
            return {
                ...
                minHeight: 700,
                dataArray: [{'cont': 'aaa', 'direction': 'send', 'type': 1, 'UserId': 111, 'userName': 111}]
            }
        },
...
<style scoped>
    ...
    .window {box-shadow: 1px 1px 20px -5px #000;  min-width: 300px; background: #F5F5F5; margin: 0 auto; overflow: hidden; padding: 0; height: 100%;position: relative;z-index: 101;}
