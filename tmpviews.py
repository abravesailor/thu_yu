...
def msglist_ajax(request):
    ...
    try:
        ...
        for msg in messages:
            ...
            tmp_dict['create_time']=msg.create_time.strftime("%Y-%m-%d %H:%M")
...
def add_notice(request):
    ...
    try:
        ...
        lesson = Lessons.objects.get(id=class_id)
        notice=Notice(notice_title=notice_title,context=context,lessons=lesson,teachers=realname)
        notice.save()
        response['status'] = 'success'
    except Exception as e:
        print(repr(e))
...
def filelist_ajax(request):
    ...
    try:
        ...
        for file in files:
            tmp_dict={}
            tmp_dict['file_name']=file.notice_title
            tmp_dict['create_time']=file.create_time.astimezone(timezone(timedelta(hours=8))).strftime("%Y-%m-%d %H:%M")
            tmp_dict['size']=file.context
            tmp_dict['field'] = file.teachers
...
def assignment_display_for_user(request):
    ...
    try:
        ...
        for item in assignment_list:
            ...
            tmp_result['modify_time']=item.modify_date
...
def  assignment_check_for_teacher(request):
    ...
    try:
        ...
        response['modify_time'] = assignment.modify_date.astimezone(timezone(timedelta(hours=8))).strftime("%Y-%m-%d %H:%M")
        response['result'] = result
        response['status'] = 'success'
    except Exception as e:
        print(repr(e))
...
def upload(request):
    ...
    try:
        ...
        file=Files(file_name=path,size=size,field=field)
